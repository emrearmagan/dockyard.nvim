...
function M.open(node)
	...
	panel_state.set_tab(ctrl.default_tab())
...
function M.next_tab()
	...
	panel_state.set_tab(tab_list[next_idx].key)
	M.render()
end
...
function M.prev_tab()
	...
	panel_state.set_tab(tab_list[prev_idx].key)
	M.render()
end
...
function M.render()
	...
	if not panel_state.current_node then
		...
		if ctrl and type(ctrl.render) == "function" then
			...
			lines = { "", "  Unknown type: " .. tostring(panel_state.current_node.kind) }
		end
	end
