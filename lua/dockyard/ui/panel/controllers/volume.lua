...
local function ensure_inspect(volume_name)
	if state.inspect_id == volume_name then
	...
	state.inspect_id = volume_name
	state.inspect_data = nil
	state.inspect_loading = true

	local requested_id = volume_name
	docker.inspect("volume", volume_name, function(result)
...
local function render_config(data, width)
	...
	local created = type(data.CreatedAt) == "string" and data.CreatedAt:sub(1, 19):gsub("T", " ") or "-"

	append_detail_section(lines, spans, "General", {
		{ "Driver", data.Driver or "-" },
		{ "Scope", data.Scope or "-" },
		{ "Created", created },
		{ "Mountpoint", data.Mountpoint or "-" },
	}, 1)
...
function M.render(width)
	...
	if item then
		local data = ensure_inspect(item.name)
		if not data then
